...
const schema = Joi.object({
  ...
  precipriceCostoCosto: Joi.number().allow(0).min(0).required().messages(messages.validationStringMessages),
  salesPrice: Joi.number().allow(0).min(0).positive().required().messages(messages.validationStringMessages),
  total: Joi.number().allow(0).min(0).positive().required().messages(messages.validationStringMessages),
